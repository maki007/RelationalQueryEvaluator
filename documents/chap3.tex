...
In this section we describe algorithm for selecting join order. We are using Two algorithms dynamic programming and greedy algorithm. For dynamic programming we use version where we enumerate all possible tree. This algorithm can give us very good plans but has exponential complexity and that's why we use it only if number if joined relations in grouped join node is small. If we join more relations we use greedy algorithm, which only generated left or right deep trees but it's complexity is not exponential only polynomial.
...
We use a variation of algorithm described in section \ref{dymanicalgorithm}. For every subset we use best plans from it's disjoin subsets to generate new plans. 
...
\subsubsection{Greedy algorithm for selection join order}
 ...
 In every following iteration for every tree we generate new trees by adding new relation to tree. In following iteration we continue only with best $k$ join trees.
