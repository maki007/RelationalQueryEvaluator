...
In the chapter we introduce some theory of relational algebra, it's optimizations and physical plan generation. Source of this theory is book Database systems\cite{database}. This informations were used for tool implementation.
...
In classical relational algebra all operators and expression results are set. All this operations can be applied also to bags. Im implemented compiler we used relational algebra based on bags.
Classical relation algebra operators are:
\begin{itemize}
...
\item Renaming operations, that doesn't change tuples of the relation bud it's changes schema.
\end{itemize}
...
We can also use renaming operations if relations doesn't have attribute names.
...
In order to control name of attributes or relation name we have renaming operator. We can use operator $\rho_{A_1=R_1,A_2=R_2,...,A_n=R_n}(R)$. Result will have the same tuples as $R$ and attributes ($R_1,R_2,...,R_n)$ will be renamed to $(A_1,A_2,...,A_n)$.
...
Let's have tuple $t$ that appears in $R$ $m$-times and $S$ $n$-times. In the Bag intersection $R \cap_B S$ will be $t$ $min(m,n)$-times.
...
We often don't want to compute aggregation function for entire column. We rather compute this function on for some group of columns. For example we can compute average salary for every person in database, or we can group them by companies and get average salary in every company. 
...
We can add projection anywhere in the tree as long as it only eliminates attributes, which are not used anymore and they aren't in query result.
